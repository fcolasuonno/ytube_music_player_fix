...
class yTubeMusicComponent(MediaPlayerEntity):
	...
	async def async_prepare_play(self):
		...
		try:
			...
			self.log_me('error', "async_get_cipher failed during prepare_play: " + str(err))
			return False
	...
	async def async_get_cipher(self, videoId):
		...
		# Use requests (urllib3) instead of pytubefix's urllib-based request.get
		# because urllib3 is patched to force IPv4
		...
		def _fetch(url):
			...
			return resp.text
		# Retry up to 3 times with backoff - youtube.com can be transiently unreachable
		last_err = None
		for attempt in range(3):
			try:
				if attempt > 0:
					await _asyncio.sleep(2 * attempt)
					self.log_me('debug', f"async_get_cipher retry {attempt}/2")
				embed_html = await self.hass.async_add_executor_job(_fetch, embed_url)
				js_url = extract.js_url(embed_html)
				self._js = await self.hass.async_add_executor_job(_fetch, js_url)
				# pytubefix >= 6.x dropped the js_url parameter from Cipher.__init__
				try:
					self._cipher = pytubefix.cipher.Cipher(js=self._js)
				except TypeError:
				...
				last_err = err
				self.log_me('debug', f"async_get_cipher attempt {attempt+1} failed: {err}")
		raise last_err
	...
	async def async_get_url(self, videoId=None, retry=60):
		...
		# NOTE: HEAD checks removed - YouTube CDN (googlevideo.com) returns 403 on HEAD
		# requests even for valid URLs, causing false negatives. The Chromecast streams
		# directly and doesn't need HA to verify the URL first.
		if(_url == ""):
	...
	async def async_get_url_self(self, videoId=None, retry=60):
		...
		try:
			...
			if(not(stop)):
				...
				# Filter out SABR streams - incompatible with Chromecast's default media receiver.
				# SABR (Server-side ABR / sabr=1) is a newer YouTube format Chromecast cannot play.
				# Also filter streams missing both 'url' and 'signatureCipher' (unplayable).
				def _is_sabr(s):
					for field in ('url', 'signatureCipher', 'cipher'):
						if 'sabr=1' in s.get(field, ''):
							return True
					return False
				non_sabr = [s for s in valid_streams if not _is_sabr(s)]
				if non_sabr:
					valid_streams = non_sabr
					self.log_me('debug', 'Filtered SABR streams, ' + str(len(valid_streams)) + ' non-SABR remaining')
				else:
					self.log_me('debug', 'All streams are SABR - keeping as fallback')
				...
				if(valid_streams[streamId].get('url') is None):
					...
					if "signature" in res['url'] or ("s" not in res and ("&sig=" in res['url'] or "&lsig=" in res['url'])):
						...
						# pytubefix renamed this method across versions:
						# <=9.x: get_signature(ciphered_signature=s) or get_signature(s)
						# 10.x+: get_sig(s)
						# some builds: decrypt(s)
						_s = res['s']
						signature = None
						for _method in ('get_sig', 'get_signature', 'decrypt'):
							_fn = getattr(self._cipher, _method, None)
							if _fn is None:
								continue
							try:
								signature = _fn(_s)
								...
								try:
									signature = _fn(ciphered_signature=_s)
									break
								except Exception:
									continue
						if signature is None:
							avail = [m for m in dir(self._cipher) if not m.startswith('_')]
							raise AttributeError(f"No signature method found on Cipher. Available: {avail}")
			...
			# Transform 'n' param (nsig) to prevent YouTube CDN throttling.
			# Without this YouTube will block or throttle the stream even with a valid sig.
			if _url and self._cipher is not None:
				try:
					...
					if 'n' in _params:
						_n_orig = _params['n'][0]
						_n_transformed = self._cipher.get_nsig(_n_orig)
						_params['n'] = [_n_transformed]
						_new_query = urlencode({k: v[0] for k, v in _params.items()})
						_url = urlunparse(_parsed._replace(query=_new_query))
						self.log_me('debug', "- nsig transformed")
				except Exception as _e:
					self.log_me('debug', "- nsig transform failed (non-fatal): " + str(_e))
